...
const MemberModal: React.FC<MemberModalProps> = ({ visible, member, onClose }) => {
  ...
  return (
    ...
    >
      {member && (
        <>
          {!isMobile && member.imgSrc && ( // 모바일이 아닐 때만 이미지 렌더링
          ...
          <p><strong>학과:</strong> {member.department}</p>
          <p><strong>직급:</strong> {member.managementTeam || "N/A"}</p>
          <p><strong>학번:</strong> {member.classOf}학번</p>
          <p><strong>추가 정보:</strong> {member.additionalInfo || "없음"}</p>
        </>
      )}
    </Modal>
  );
